...
def error_auc(errors, thresholds):
    """
    计算误差的 AUC (Area Under Curve)
    Args:
        errors (list/ndarray): 所有的误差值 [N,]
        thresholds (list): AUC 整合的阈值列表，如 [3, 5, 10]
    Returns:
        results (dict): {auc@3: val, auc@5: val, ...}
    """
...
def calculate_metrics(img_origin, img_result, mkpts0, mkpts1, kpts0=None, kpts1=None, ctrl_pts0=None, ctrl_pts1=None, H_gt=None):
    ...
    if H_pred is not None and ctrl_pts0 is not None and ctrl_pts1 is not None and len(ctrl_pts0) > 0:
        ...
        # 2.1 计算 AUC (Area Under Curve) 
        # 使用 control points 的 reprojection errors
        auc_results = error_auc(errors, [3, 5, 10])
