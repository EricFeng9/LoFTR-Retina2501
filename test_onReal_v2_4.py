...
# 数据集根目录 (保持与 training scripts 一致使用相对路径)
DATA_ROOTS = {
    'cffa': "data/operation_pre_filtered_cffa",
    'cfoct': "data/operation_pre_filtered_cfoct",
    'octfa': "data/operation_pre_filtered_octfa",
    'cfocta': "data/CF_OCTA_v2_repaired"
}
...
def main():
    ...
    if args.minima:
        # MINIMA 权重路径 (相对路径)
        ckpt_path = Path("third_party/MINIMA/weights/minima_loftr.ckpt")
    else:
    ...
    if args.minima:
        ...
        # 使用 PL_LoFTR_V3 加载以对齐 v2.4 的训练类名
        model = PL_LoFTR_V3.load_from_checkpoint(str(ckpt_path), config=config)
    model.cuda().eval()
    ...
    for i, batch in enumerate(tqdm(test_loader, desc="Testing")):
        ...
        logger.info(f"Sample: {pair_name} | SR_ME: {metrics['SR_ME']} | MeanErr: {metrics['mean_error']:.2f} | AUC@3: {metrics['auc@3']:.4f} | AUC@5: {metrics['auc@5']:.4f} | AUC@10: {metrics['auc@10']:.4f}")
