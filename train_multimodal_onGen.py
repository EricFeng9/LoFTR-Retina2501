...
# 数据集根目录硬编码
DATA_ROOT = "/data/student/Fengjunming/LoFTR/data/CF_260116_expand"
    ...
    def setup(self, stage=None):
        if stage == 'fit' or stage is None:
            ...
            self.train_dataset = MultiModalDataset(
                DATA_ROOT, mode=self.args.mode, split='train', img_size=self.args.img_size)
            # 验证集：形变固定（Dataset内部通过idx固定种子实现）
            self.val_dataset = MultiModalDataset(
                DATA_ROOT, mode=self.args.mode, split='val', img_size=self.args.img_size)
